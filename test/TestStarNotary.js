...
it('can add the star name and star symbol properly', async() => {
    ...
    await instance.createStar('New Start', starId, {from: user1});
    // console.log(instance)

    //2. Call the name and symbol properties in your Smart Contract and compare with the name and symbol provided
    assert.equal(await instance.tokenName(), "My Token");
...
it('lets a user transfer a star', async() => {
    ...
    await instance.createStar('awesome star1', starId1, {from: user1});
    await instance.createStar('awesome star2', starId2, {from: user1});
    // 1. create a Star with different tokenId
    ...
    assert.equal(await instance.ownerOf(starId1), user2);
    assert.notEqual(await instance.ownerOf(starId1), user1);
});
...
it('lookUptokenIdToStarInfo test', async() => {
    ...
    await instance.createStar('awesome star13', starId, {from: user1});

    // 2. Call your method lookUptokenIdToStarInfo
    const starName = await instance.lookUptokenIdToStarInfo(starId, {from: user1});
    // 3. Verify if you Star name is the same
    assert.equal(starName, 'awesome star13');
});